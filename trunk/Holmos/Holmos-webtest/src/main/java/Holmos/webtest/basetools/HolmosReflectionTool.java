...
public class HolmosReflectionTool {
	 ...
	 * @param typeForInstance 类型的Class对象
	 * @param isAccessible true 改变构造函数的访问控制,如果构造函数的访问权不是public的话,不抛出异常
	 * <br> false 不改变访问权限,访问权限不够的话抛出异常
	 * @return 构造出的变量实例*/
	...
	public static Object createInstanceOfType(Class typeForInstance,
		...
		if(typeForInstance.isMemberClass()||Modifier.isStatic(typeForInstance.getModifiers())){
			/*如果是成员类(内部类的一种)或者是静态类的时候,不能实例化;原因是成员内部类在外层类的外部是不可见的,是一种
			  比private修饰符还要严格的一种形式;所以无法用这种方法实例化;对于静态类:由于静态类仅仅能包含静态成员,并且
			  不能用new 关键字生成静态类对象,所以这种类型也不支持
			  */
			throw new HolmosFailedError("对于成员内部类和静态类，Holmos框架无法为其创建对象实例!详情请见java中关于成员内部类和" +
		...
		try{
			...
			throw new HolmosFailedError("创建" + typeForInstance.getName()+"对象的时候发生错误!");
	 ...
	 * @param object 给定的字段母体
	 * @param fields 字段列表,此列表中的任何一个字段都必须是object里面的字段
	 * @param value 给字段赋的值
	 * 
	 * */
	...
	/**通过给定的setter方法给字段赋值
	 * @param object 给定的setter方法母体
	 * @param setter 给定的setter方法
	 * @param value 通过setter方法赋的值
	 * */
	...
	/**校验method是否为setter方法,校验规则为满足下面三个条件:
	 * <li>方法名长度大于3</li>
	 * <li>方法名以set开头,并且方法名的第四个字母为大写</li>
	 * <li>方法的参数个数只有一个</li>
	 * @param method 待校验的方法对象
	 * @return true 是setter方法 <br> false 不是setter方法
	 * */
	private static boolean isSetterMethod(Method method){
		String methodName=method.getName();
		if(methodName.length()>3||methodName.startsWith("set")&&method.getParameterTypes().length==1){
			if(methodName.substring(3, 4).indexOf(0)>='A'&&methodName.substring(3, 4).indexOf(0)<='Z')
			return true;
		}return false;
	}
