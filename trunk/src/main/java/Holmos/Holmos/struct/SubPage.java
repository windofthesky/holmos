...
public class SubPage implements LocatorValue{
	...
	private boolean isSubPageExist(int waitCount){
		...
		if(currentWindow instanceof SeleniumBrowserWindow)
			return isSeleniumSubPageExist(waitCount);
		else if(currentWindow instanceof WebDriverBrowserWindow)
			return isWebDriverSubPageExist(waitCount);
		return false;
	}
	private boolean isSeleniumSubPageExist(int waitCount){
		int count=0;
		...
		if(infoChain.getInfoNodes().size()<=1){
			while(count++<waitCount){
				if(findSeleniumSubPage(seleniumEngine))
					return true;
			}
		}else{
	...
	private boolean findSeleniumSubPage(Selenium seleniumEngine){
		if(findSeleniumSubPageById(seleniumEngine))return true;
		if(findSeleniumSubPageByName(seleniumEngine))return true;
		if(findSeleniumSubPageByXpath(seleniumEngine))return true;
		if(findSeleniumSubPageByCss(seleniumEngine))return true;
		if(findSeleniumSubPageByLinkText(seleniumEngine))return true;
		if(findSeleniumSubPageByRegularLinkText(seleniumEngine))return true;
		return false;
	}
	private boolean findSeleniumSubPageById(Selenium seleniumEngine){
	...
	private boolean findSeleniumSubPageByName(Selenium seleniumEngine){
	...
	private boolean findSeleniumSubPageByXpath(Selenium seleniumEngine){
	...
	private boolean findSeleniumSubPageByCss(Selenium seleniumEngine){
	...
	private boolean findSeleniumSubPageByLinkText(Selenium seleniumEngine){
	...
	private boolean findSeleniumSubPageByRegularLinkText(Selenium seleniumEngine){
	...
	private boolean findWebDriverSubPage(LocatorFinder finder){
		if(findWebDriverSubPageById(finder))return true;
		if(findWebDriverSubPageByName(finder))return true;
		if(findWebDriverSubPageByXpath(finder))return true;
		if(findWebDriverSubPageByCss(finder))return true;
		if(findWebDriverSubPageByLinkText(finder))return true;
		if(findWebDriverSubPageByPartialLinkText(finder))return true;
		if(findWebDriverSubPageByTagName(finder))return true;
		return false;
	}
	private boolean findWebDriverSubPageById(LocatorFinder finder){
		if(locator.getLocatorById()==null||"".equalsIgnoreCase(locator.getLocatorById()))
			return false;
		try{
	...
	private boolean findWebDriverSubPageByName(LocatorFinder finder){
		if(locator.getLocatorByName()==null||"".equalsIgnoreCase(locator.getLocatorByName()))
			return false;
		try{
	...
	private boolean findWebDriverSubPageByXpath(LocatorFinder finder){
		if(locator.getLocatorByXpath()==null||"".equalsIgnoreCase(locator.getLocatorByXpath()))
			return false;
		try{
	...
	private boolean findWebDriverSubPageByCss(LocatorFinder finder){
		if(locator.getLocatorByCSS()==null||"".equalsIgnoreCase(locator.getLocatorByCSS()))
			return false;
		try{
	...
	private boolean findWebDriverSubPageByLinkText(LocatorFinder finder){
		if(locator.getLocatorByLinkText()==null||"".equalsIgnoreCase(locator.getLocatorByLinkText()))
			return false;
		try{
	...
	private boolean findWebDriverSubPageByPartialLinkText(LocatorFinder finder){
	...
	private boolean findWebDriverSubPageByTagName(LocatorFinder finder){
	...
	public void waitForDisppear() {
		...
		if(currentWindow instanceof SeleniumBrowserWindow){
			while(waitCount++<ConfigConstValue.waitCount){
				if(!findSeleniumSubPage((Selenium) currentWindow.getDriver().getEngine()))
					return;
			}
		}else if(currentWindow instanceof WebDriverBrowserWindow){
